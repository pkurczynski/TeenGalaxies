...
THIS VERSION FOR 2015-05-16-A/sample01/run03b  lin-exp SFH FITS.
...
if __name__ == '__main__':        
    ...
    *fits done without covariance. (2) is the best fit 
    ...
    """
    if (args.verbose):
       refit_data_table_id_cov_corr_theta_filename = 'sfr_life_vs_m_refit_id_cov_corr_ellipse.txt'    
    ...
    x_selection_function = np.linspace(5.0,8.0,1000)  
    ...
    # label a 3 sigma upper limit where the formal scatter estimate is zero
    text_label_scatter =r"$\sigma_{is} < " + r"{:.3f}".format(3*np.sqrt(f87_rescaled_refit_scatter_variance_error))+r"$"
    ...
    if (args.verbose):  
        ...
        print("\nsfr_life_vs_m_line_clip_sim_linexp.py Done!")