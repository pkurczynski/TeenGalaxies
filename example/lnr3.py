...
def bces(x1, x2, x1err=[], x2err=[], cerr=[], logify=True, model='yx', \
    ...
    if full_output:
        out = ((a[j], scipy.sqrt(avar[j])),
               (b[j], scipy.sqrt(bvar[j])),
               covar_ab)
    else:
        out = ((a[j], scipy.sqrt(avar[j])),
               (b[j], scipy.sqrt(bvar[j])))
...
def scatter(slope, zero, x1, x2, x1err=[], x2err=[]):
    ...
    if len(x2err) == n:
        ...
        s0 = s - s_obs
    print( scipy.sqrt(s), scipy.sqrt(s_obs), scipy.sqrt(s0))
    return scipy.sqrt(s0)
...
def kelly(x1, x2, x1err=[], x2err=[], cerr=[], logify=True,
    ...
    sigma = scipy.sqrt(idl.ev('fit.sigsqr'))
...
def mcmc(x1, x2, x1err=[], x2err=[], po=(1,1,0.5), logify=True,
    ...
    def lnlike(theta, x, y, xerr, yerr):
        a, b, s = theta
        model = a + b*x
        sigma = scipy.sqrt((b*xerr)**2 + yerr*2 + s**2)
        lglk = 2 * sum(scipy.log(sigma)) + \
               sum(((y-model) / sigma) ** 2) + \
               scipy.log(len(x)) * scipy.sqrt(2*scipy.pi) / 2
        return -lglk
    def lnprior(theta):
        a, b, s = theta
        if s >= 0:
    ...
    if logify:
        x1, x2, x1err, x2err = to_log(x1, x2, x1err, x2err)
    start = scipy.array(po)
    ndim = len(start)
    pos = [start + 1e-4*scipy.random.randn(ndim) for i in range(nwalkers)]
    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob,
                                    args=(x1,x2,x1err,x2err))
    sampler.run_mcmc(pos, nsteps)
    samples = scipy.array([sampler.chain[:,nburn:,i].reshape(-1) \
                            for i in range(ndim)])
    if logify:
        samples[2] *= scipy.log(10)
    if output == 'full':
        return samples
    if type(output) in (int, float, numpy.float64):
        values = [[scipy.percentile(s, o) for o in output] for s in samples]
        return values
...
def mle(x1, x2, x1err=[], x2err=[], cerr=[], s_int=True,
    ...
    f = lambda a, b: a + b * x1
    if s_int:
        w = lambda b, s: scipy.sqrt(b**2 * x1err**2 + x2err**2 + s**2)
        loglike = lambda p: 2 * sum(scipy.log(w(p[1],p[2]))) + \
                            sum(((x2 - f(p[0],p[1])) / w(p[1],p[2])) ** 2) + \
                            scipy.log(n * scipy.sqrt(2*scipy.pi)) / 2
    else:
        w = lambda b: scipy.sqrt(b**2 * x1err**2 + x2err**2)
        loglike = lambda p: sum(scipy.log(w(p[1]))) + \
