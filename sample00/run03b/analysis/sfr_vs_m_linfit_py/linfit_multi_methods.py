...
v6 09/17/2015 include covariance in adhoc scatter estimate
pk
"""
import lnr3 as lnr
...
def scatter_variance_adhoc(x,y,xerr,yerr,slope,intercept, covxy = None, verbose = False):
    """
    ...
    Parameters:
        ...
        cov (float array):  covariance values for each x,y point.
    ...
    if covxy is not None:
        error_covariance_term = 2 * slope * covxy.mean()
        intrinsic_scatter_variance_term = total_scatter_variance - yerr_scatter_term - slope**2 * xerr_scatter_term  + error_covariance_term
    else:
    ...
    if verbose:
        ...
        if covxy is not None:
            print("\tCovariance term, 2m<Cov(x,y)>          : {:.5f}".format(error_covariance_term))
