#!/usr/bin/env python
"""
sfr_life_vs_m_line_clip_sim_linexp.py                       Python3 script

THIS VERSION FOR 2015-05-16-A/sample00/run03  lin-exp SFH FITS.
...
v7 07/31/2015  error ellipse estimation.  Corrected error in accessing
                covariance of input data, see Table 1.
"""
...
if __name__ == '__main__':        
    ...
    if (args.verbose):
        print("sfr_life_vs_m_line_clip_sim_linexp.py \n")
    ...
    Covariance_LogStellarMass_LogSFR = tbData.field('Cov_8_12')
    Variance_LogStellarMass = tbData.field('Cov_8_8')
    Variance_LogSFR = tbData.field('Cov_12_12')

    Chisq_BestFit = tbData.field('Chisq_BestFit')
    
    Correlation_LogStellarMass_LogSFR = tbData.field('Corr_8_12')
    ...
    if (args.verbose):
       ...
       """
       db_import_filename = 'sfr_life_vs_m_refit_db_table.txt'
    ...
    # that is a straight line.  Prevent numerical error in sqrt()
    sigyp2 = max(0.5 * (sigx**2 + sigy**2) - sigxyterm, 0.0)
    semi_major_axis = np.sqrt(sigxp2)
    semi_minor_axis = np.sqrt(sigyp2)
    # arctan2(y/x):  "y" is the first parameter, "x" is the second parameter    
    theta = np.degrees(0.5 * np.arctan2(2*sigxy,(sigx**2 - sigy**2))) 
    #theta = np.degrees(0.5 * np.arctan2((sigx**2 - sigy**2),2*sigxy)) 
    ...
    eg. x_bins = np.array([1.0, 8.0, 9.0, 10, 20.0])
        where   bin1 = [1.0, 8.0]
                bin2 = [8.0, 9.0]
                bin3 = [9.0, 10.0]
                bin4 = [10.0, 20.0]
    """
    x_bins = np.array([1.0, 8.0, 9.0, 10, 20.0])
    ...
    """
    assign y values in each of 4 x bins (for boxplot).
    ...
    y_binscat_bin4 = y_binscat[(x_indices == 4)]
    y_boxplot_data = [y_binscat_bin1, y_binscat_bin2, y_binscat_bin3, y_binscat_bin4]
    ...
    if (args.verbose):
        ...
        print("\nBin 4")
        print("\tNumber of data in bin          : ", y_binscat_bin3.size)
        print("\tx Range                        : ", x_bins[3:5])
        print("\tx Bin     - Mean               : {:.2f}".format(mean_x_bin[4]))
        print("\tx Bin     - Median             : {:.2f}".format(median_x_bin[4]))
        print("\tx Bin     - Std Dev            : {:.2f}".format(std_x_bin[4]))   
        print("\tResidual - Mean           : {:.2f}".format(y_binscat_bin3.mean()))
        print("\tResidual - Median         : {:.2f}".format(np.median(y_binscat_bin4)))
        print("\tResidual - Std Dev        : {:.2f}".format(np.std(y_binscat_bin4)))
